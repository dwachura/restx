...
/**
 * Base interface used to flag classes that represents valid error response payload.
 */
 ...
 */
data class OperationError(override val code: String, override val message: String) : SingleErrorPayload
...
class Source private constructor(val type: Type, val location: String) {
    ...
    companion object Factories {
        fun queryParam(name: String) = Source(Type.QUERY, name)
        fun header(name: String) = Source(Type.HEADER, name)
        fun bodyProperty(path: String) = Source(Type.BODY, path)
    }
